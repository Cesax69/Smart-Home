...
exports.buildQueryWithAI = void 0;
const axios_1 = __importDefault(require("axios"));
const buildQueryWithAI = async ({ message, schema, connection, userId, userRole, userName, email }) => {
    const system = `
...
exports.buildQueryWithAI = buildQueryWithAI;
const inferMongoCollection = (schema, message) => {
...
const resolveUserIdFromUsersService = async (userName, email) => {
    try {
        ...
        const { data } = await axios_1.default.get(`${base}/users`);
        const list = Array.isArray(data?.data) ? data.data : [];
        const uName = (userName || '').toLowerCase();
        const uEmail = (email || '').toLowerCase();
        const found = list.find((u) => (uName && String(u.username || '').toLowerCase() === uName) ||
...
const buildHeuristicQuery = async ({ message, schema, connection, userId, userRole, userName, email }) => {
    const msg = message.toLowerCase();
    const tables = Object.keys(schema?.schema || {});
    const hasTable = (name) => tables.some(t => t.toLowerCase() === name.toLowerCase());
    const pickTable = (predicate, fallback) => {
        const found = tables.find(predicate);
        return found || (hasTable(fallback) ? fallback : (tables[0] || fallback));
    };
    const scoreTableForUser = (t) => {
        const cols = (schema?.schema?.[t] || []).map((c) => c.column?.toLowerCase?.());
        let score = 0;
        ...
        if (cols.includes('last_name'))
            score++;
        if (cols.includes('family_role_id'))
            score++;
        return score;
    };
    if (connection.type === 'mongo') {
    ...
    const isUserIntent = (msg.includes('usuario') ||
    ...
    if (isUserIntent) {
        ...
        const bestUser = tables.reduce((best, t) => {
        ...
        const userCols = (schema?.schema?.[usersTable] || []).map((c) => c.column);
        const roleCols = (schema?.schema?.[rolesTable] || []).map((c) => c.column);
    ...
    const bestTasks = tables.reduce((best, t) => {
    ...
    const hasMyIntent = (/\b(mi|mis|mio|mío|mía|mias|mías|míos)\b/.test(msg) ||
        msg.includes('tengo') ||
    ...
    if (userRole !== 'head_of_household' && hasMyIntent) {
        const safe = (v) => (v || '').replace(/'/g, "''").toLowerCase();
        ...
        // 2) Si NO existe: resolvemos el ID llamando al users-service y filtramos por ID
